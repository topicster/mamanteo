...
% Define a river flow diversion factor for the regional analysis
userinput_fdv = 0.10;
...
% Display input data
disp('The considered mamanteo functioning parameters are:')
disp('Minimum flows left in the stream [l/s/km2]')
disp(userinput_QMin)
disp('Maximum mamanteo intake capacity [l/s]')
disp(userinput_QMax)
...
% Volumes in mm
ANA_CHO_HQ_1day(:,4)     = ANA_CHO_HQ_1day(:,3)*1e9/(ANA_CHO_AREA*1e6);
% Calculate baseflow
...
sp1 = subplot(2,2,1);
hold on
bar(mean(SENAMHI_HMT_PP_19902018_Hmon),'facecolor',0.8*[1 1 1],'DisplayName','Highlands''s precipitation (1990-2018)')
boxplot(SENAMHI_HMT_PP_19902018_Hmon,'PlotStyle','compact','colors',0*[1 1 1],'Labels',MonthLabels)
plot(SENAMHI_HMT_PP_DThrmon_19902018([refmon_hy:12,1:refmon_hy-1]),'-r','Linewidth',1,'DisplayName','Highlands''s precipitation drought threshold')
% Plot settings
...
sp2 = subplot(2,2,2);
hold on
bL = bar(mean(SENAMHI_CHO_PP_19902018_Hmon),'facecolor',0.3*[1 1 1],'DisplayName','Lima''s precipitation (1990-2018)');
boxplot(SENAMHI_CHO_PP_19902018_Hmon,'PlotStyle','compact','colors',0*[1 1 1],'Labels',MonthLabels)
dL   = plot(SENAMHI_CHO_PP_DThrmon_19902018([refmon_hy:12,1:refmon_hy-1]),'--r','Linewidth',1,'DisplayName','Lima''s precipitation drought threshold');
%-- For legend
bH = bar(-12:-1,mean(SENAMHI_HMT_PP_19902018_Hmon),'facecolor',0.8*[1 1 1],'DisplayName','Highlands''s precipitation (1990-2018)');
dH = plot(-12:-1,SENAMHI_HMT_PP_DThrmon_19902018([refmon_hy:12,1:refmon_hy-1]),'-r','Linewidth',1,'DisplayName','Highlands''s precipitation drought threshold');
%--
% Plot settings
set(gca,'Xlim',[0.5 12.5],...
    'Ylim',[-offset_loc(3) 1+offset_loc(3)]*300,...
...
% plot(userinput_frl*ANA_CHO_HQ_DThrmon_19211960([refmon:12,1:refmon-1])*ANA_CHO_AREA/1e3,'-.m','Linewidth',1.5,'DisplayName','Natural Rimac river flow drought threshold')
D = plot(userinput_frl*ANA_CHO_HQ_DThrmon_19902018([refmon_hy:12,1:refmon_hy-1])*ANA_CHO_AREA/1e3,':r','Linewidth',2,'DisplayName','Altered Rimac river flow drought threshold');
...
b(1).DisplayName = 'Highlands''s annual precipitation';
b(2).DisplayName = 'Lima''s annual precipitation';
% Thresholds
plot([refyrs_hy(1) refyrs_hy(2)+1],[1 1]*SENAMHI_HMT_PP_DThryr_19902018(2),'-r','Linewidth',1,'DisplayName','Highlands''s drought threshold')
plot([refyrs_hy(1) refyrs_hy(2)+1],[1 1]*SENAMHI_CHO_PP_DThryr_19902018(2),'--r','Linewidth',1,'DisplayName','Lima''s drought threshold')
...
disp('Done')
disp(' ')
%% 7. Diverted water volumes
disp('7. Estimating potential diverted water volumes')
...
% Extend the tracer data to deplet after one year
...
% Ponderate relative to the baseflow and diverted flow
...
disp('9. Estimating the modified hydrograph affected by mamanteo')

% Residence times at 5-min scale
l5mbf = iMHEA_HMT_Tracers_Int_5min(:,2);
...
plot(iMHEA_HMT_Tracers_Int_1day(:,1)-iMHEA_HMT_Tracers_Avg(1,1),...
    ...
    'DisplayName','1-day interpolated residence time')
plot(iMHEA_HMT_Tracers_Int_1day(:,1)-iMHEA_HMT_Tracers_Avg(1,1),...
    iMHEA_HMT_Tracers_Int_1day(:,5),...
    '--','LineWidth',1,'color',[0 0.5 0],...
    'DisplayName','1-day scaled by baseflow')
...
% Plot settings
set(gca,'YGrid','on','TickDir','out','Ylim',[-0.01 1.01])
box off
% Labels
...
plot(iMHEA_HMT_Tracers_MTT(3)*[1 1],[-.1 0.50],...
    ':k')
plot([0 iMHEA_HMT_Tracers_MTT(3)],0.50*[1 1],...
    ':k')
plot(iMHEA_HMT_Tracers_MTT(1)*[1 1],[-.1 aux_loct],...
    ':r')
plot([0 iMHEA_HMT_Tracers_MTT(1)],aux_loct*[1 1],...
    ':r')
...
plot(datetime(iMHEA_HMT_Tracers_Int_1day(:,1),'ConvertFrom','datenum'),...
    ...
    'color',[0.08 0.17 0.55],'DisplayName','Baseflow (proxy for spring flow)')
% Plot settings
set(gca,'YGrid','on','TickDir','out','Ylim',[-0.05 1.05]*1)
...
plot(datetime(iMHEA_HMT_Tracers_Int_1day(:,1),'ConvertFrom','datenum'),...
    ...
    '-','LineWidth',1,'color',[0.85 0.33 0.01],...
    'DisplayName','1-day interpolation')
% Plot settings
set(gca,'YGrid','on','TickDir','out','Ylim',[-0.05 1.05]*10)
ax17 = get(gca,'Xlim');
datetick('x','mmm-yy','keeplimits','keepticks')
box off
% Labels
...
plot(datetime(iMHEA_HMT_Tracers_Int_1day(:,1),'ConvertFrom','datenum'),...
    ...
    '--','LineWidth',1,'color',[0 0.5 0],...
    'DisplayName','1-day residence time scaled by baseflow')
% Plot settings
set(gca,'YGrid','on','TickDir','out','Ylim',[-0.05 1.05]*0.03)
datetick('x','mmm-yy','keeplimits','keepticks')
box off
% Labels
...
plot(datetime(iMHEA_HMT_01_HQ_1day_mm(:,1),'ConvertFrom','datenum'),...
    iMHEA_HMT_01_HQ_1day_mm(:,2),...
    'LineWidth',1,'DisplayName','Original discharge')
plot(datetime(iMHEA_HMT_01_MAMFLOW_1day(:,1),'ConvertFrom','datenum'),...
    iMHEA_HMT_01_MAMFLOW_1day(:,4),...
    'LineWidth',1,'DisplayName','Modified hydrograph')
% Plot settings
set(gca,'YGrid','on','TickDir','out','Ylim',[-0.05 1.05]*10,'Xlim',ax17)
datetick('x','mmm-yy','keeplimits','keepticks')
box off
% Labels
ylabel('[mm day{-1}]')
...
% Export tracer Figure
disp('Exporting the Figure to pdf')
% Set the paper size [width height]
set(fig_S3,'PaperSize',[30 25]);
% set(fig_S3,'PaperOrientation','landscape')
print(fig_S3,'FigS3_export','-dpdf','-fillpage')

% Clear axuliar variables
clear fig_S3 offset_loc ax17 aux_loct
...
subplot(1,2,1)
hold on
plot(refhydt,iMHEA_HMT_01_MAMFLOW_Hmon(:,1),'Linewidth',1,'DisplayName','Catchment C1 rainfall');
plot(refhydt,iMHEA_HMT_01_MAMFLOW_Hmon(:,2),'Linewidth',1,'DisplayName','Original C1 discharge');
plot(refhydt,iMHEA_HMT_01_MAMFLOW_Hmon(:,3),'Linewidth',1,'DisplayName','Potential effect of mamanteo');
% Plot settings
...
subplot(1,2,2)
hold on
plot(refhydt,iMHEA_HMT_02_MAMFLOW_Hmon(:,1),'Linewidth',1,'DisplayName','Catchment C2 rainfall');
plot(refhydt,iMHEA_HMT_02_MAMFLOW_Hmon(:,2),'Linewidth',1,'DisplayName','Original C2 discharge');
plot(refhydt,iMHEA_HMT_02_MAMFLOW_Hmon(:,3),'Linewidth',1,'DisplayName','Potential effect of mamanteo');
% Plot settings
set(gca,'Ylim',[-offset_loc(3) 1+offset_loc(3)]*lim1,...
    'YGrid','on',...
    'TickDir','out')
...
legend('location','Northeast')
legend('boxoff')
legend([a,f,b,c],'Catchment outlet discharge',...
    'Baseflow','Minimum flow left in the stream',...
...
clear a f b c cS dS eS fS R a2
%--
...
% (Re)define estimated recovery rate?
userinput_frr = 0.50;

% Residence times at 5-min scale
l1dbf = iMHEA_HMT_Tracers_Int_1day(:,2);
...
ANA_CHO_HQ_19211960_1day(:,2) = ANA_CHO_HQ_19211960_1day*1e9/(ANA_CHO_AREA*1e6);

% Altered flow regime with grey infrastructure 1961 - present
refyrs_hy = year(userinput_ax4);
% Reorganise in hydrological years
ANA_CHO_HQ_19612018_Hmon = iMHEA_HydroYear(ANA_CHO_HQ_Mmon,ANA_CHO_HQ_1yr(:,1),userinput_ax4);
% Extract daily data
ANA_CHO_HQ_19612018_1day = ANA_CHO_HQ_1day(and(year(ANA_CHO_HQ_DDATE)>=refyrs_hy(1),year(ANA_CHO_HQ_DDATE)<=refyrs_hy(2)),3);
ANA_CHO_HQ_19612018_date = ANA_CHO_HQ_DDATE(and(year(ANA_CHO_HQ_DDATE)>=refyrs_hy(1),year(ANA_CHO_HQ_DDATE)<=refyrs_hy(2)));
ANA_CHO_HQ_19612018_1day(:,2) = ANA_CHO_HQ_19612018_1day*1e9/(ANA_CHO_AREA*1e6);

% Matrix ANA_CHO_MAMFLOW_XXXXYYYY_1day
% column 1: diverted volumes at daily scale [m3/s]
% column 2: remaning volumes at daily scale [m3/s]
% column 3: modified hydrograph at daily scale [m3/s]
% column 3: modified hydrograph at daily scale [mm]
...
% Extract 10% of flow during the wet seasons
ANA_CHO_MAMFLOW_19612018_1day = zeros(size(ANA_CHO_HQ_19612018_1day(:,1)));
for k = 1:n
    % Fill with data only between wet season limits
    ANA_CHO_MAMFLOW_19612018_1day(and(ANA_CHO_HQ_19612018_date>ANA_CHO_ws_19602018(k,1),ANA_CHO_HQ_19612018_date<ANA_CHO_ws_19602018(k,2))) = userinput_fdv*ANA_CHO_HQ_19612018_1day(and(ANA_CHO_HQ_19612018_date>ANA_CHO_ws_19602018(k,1),ANA_CHO_HQ_19612018_date<ANA_CHO_ws_19602018(k,2)));
end
% Flows remaining in the stream
ANA_CHO_MAMFLOW_19612018_1day(:,2) = ANA_CHO_HQ_19612018_1day(:,1) - ANA_CHO_MAMFLOW_19612018_1day(:,1);
% Apply diverted flows, residence times, and recovery rate
[ANA_CHO_MAMFLOW_19612018_1day(:,3)] = iMHEA_ModifiedFlowsBF(ANA_CHO_MAMFLOW_19612018_1day(:,2),ANA_CHO_MAMFLOW_19612018_1day(:,1),ANA_CHO_HQ_1day(:,5),l1dbf,userinput_frr);
ANA_CHO_MAMFLOW_19612018_1day(:,4) = ANA_CHO_MAMFLOW_19612018_1day(:,3)*1e9/(ANA_CHO_AREA*1e6);

% Monthly modified hydrograph
[~,ANA_CHO_MM_19612018_1yr,~,~,ANA_CHO_MM_19612018_Mmon] = iMHEA_MonthlyRain(ANA_CHO_HQ_19612018_date,ANA_CHO_MAMFLOW_19612018_1day(:,3));
% Reorganise in hydrological years
...
% column 2:     mean potential diversion (using userinput_fdv % diversion)
% column 3:     mean remaming flow (approximately column 1 - column 2)
% column 4:     mean modified flow (using userinput_frr recovery rate)
% row 1:        units in m3/s
...
ANA_CHO_MAMVOL_mean(1,2:4) = aux_mean(1:3);
% million m3/yr
aux_sum = nansum(ANA_CHO_MAMFLOW_19612018_1day);
ANA_CHO_MAMVOL_mean(2,1) = nansum(ANA_CHO_HQ_19612018_1day(:,1))/n;
ANA_CHO_MAMVOL_mean(2,2:4) = aux_sum(1:3)/n;
% mm/yr
aux_factor = 1000/(ANA_CHO_AREA/1e6)/1e6/n;
ANA_CHO_MAMVOL_mean(3,1) = nansum(ANA_CHO_HQ_19612018_1day(:,2))/n;
ANA_CHO_MAMVOL_mean(3,2:3) = aux_sum(1:2)*aux_factor;
ANA_CHO_MAMVOL_mean(3,4) = aux_sum(4)/n;
...
[ANA_CHO_MM_DIndices_19612018_vs_19211960] = iMHEA_DroughtIndices(ANA_CHO_HQ_19612018_date(and(ANA_CHO_HQ_19612018_date>=userinput_ax4(1),ANA_CHO_HQ_19612018_date<=userinput_ax4(2))),ANA_CHO_MAMFLOW_19612018_1day(and(ANA_CHO_HQ_19612018_date>=userinput_ax4(1),ANA_CHO_HQ_19612018_date<=userinput_ax4(2)),4),ANA_CHO_HQ_DThr_19211960,drought_aggr_Q,1);
...
[ANA_CHO_MM_DIndices_19211960,ANA_CHO_MM_DThr_19211960] = iMHEA_Drought(ANA_CHO_HQ_19612018_date(and(ANA_CHO_HQ_19612018_date>=userinput_ax4(1),ANA_CHO_HQ_19612018_date<=userinput_ax4(2))),ANA_CHO_MAMFLOW_19612018_1day(and(ANA_CHO_HQ_19612018_date>=userinput_ax4(1),ANA_CHO_HQ_19612018_date<=userinput_ax4(2)),4),drought_method,drought_aggr_Q,1);

% Streamflow drought indices compared against 1960-2018 threshold
[ANA_CHO_MM_DIndices_19612018_vs_19612018] = iMHEA_DroughtIndices(ANA_CHO_HQ_19612018_date(and(ANA_CHO_HQ_19612018_date>=userinput_ax4(1),ANA_CHO_HQ_19612018_date<=userinput_ax4(2))),ANA_CHO_MAMFLOW_19612018_1day(and(ANA_CHO_HQ_19612018_date>=userinput_ax4(1),ANA_CHO_HQ_19612018_date<=userinput_ax4(2)),4),ANA_CHO_HQ_DThr_19612018,drought_aggr_Q,1);
...
% Natural flow regime 1921 - 1960
disp('Calculating flow durations curves for 1921 - 1960')
...
disp('Calculating flow durations curves for 1961 - present')
% Define hydrological years seasons
refyrs_hy = year(userinput_ax4);
refmon_hy = month(userinput_ax4(1,:));
n = refyrs_hy(2)-refyrs_hy(1);
ANA_CHO_hy_19602018 = [datetime([(refyrs_hy(1):refyrs_hy(2)-1)',refmon_hy(1)*ones(n,1),ones(n,1)]),datetime([(refyrs_hy(1)+1:refyrs_hy(2))',refmon_hy(2)*ones(n,1),30*ones(n,1)])];
% Calculate annual flow duration curves (FDC)
[ANA_CHO_HQ_19612018_1day_FDC] = iMHEA_AnnualFDC(ANA_CHO_HQ_DDATE,ANA_CHO_HQ_1day(:,4),ANA_CHO_hy_19602018);
[ANA_CHO_MM_19612018_1day_FDC] = iMHEA_AnnualFDC(ANA_CHO_HQ_19612018_date,ANA_CHO_MAMFLOW_19612018_1day(:,4),ANA_CHO_hy_19602018);

% Drought and demand thresholds
disp('Calculating flow durations curves for thresholds')
[~,~,~,~,ANA_CHO_HQ_DThr_19211960_FDC] = iMHEA_FDC(ANA_CHO_HQ_DThr_19211960);
[~,~,~,~,FT_RIMAC_HQ_dem_FDC] = iMHEA_FDC(FT_RIMAC_HQ_dem*1e9/(ANA_CHO_AREA*1e6)./eomday(2018,[9:12,1:8]));
...
ANA_CHO_MM_19612018_1day_FDCvar = [ANA_CHO_HQ_19612018_1day_FDC(1:end,1)',ANA_CHO_HQ_19612018_1day_FDC(end:-1:1,1)'];
...
disp('Done')
disp(' ')
%% 16. Figure: Potential effects
disp('16. Creating Figure of potential effects')
...
legend([b,c],'Original average discharge in Huamantanga',...
    'Potential effect of mamanteo (local effects)')
clear b c
% Plot seasons
...
D = plot(userinput_frl*ANA_CHO_HQ_DThrmon_19211960([refmon_hy:12,1:refmon_hy-1])*ANA_CHO_AREA/1e3,':k','Linewidth',1);
...
legend([nat,grn,D,d],'Natural flow regime in Rimac',...
    'Potential effect of mamanteo (green+grey infrastructure)',...
    'Rimac river flow drought threshold',...
    'Lima city''s water demand')
clear nat alt grn d D
% Plot seasons
...
patch(ANA_CHO_HQ_19211960_1day_FDCvar(1,:),ANA_CHO_HQ_19612018_1day_FDCvar(2,:),'r','Edgecolor','none','FaceAlpha',.1)
patch(ANA_CHO_HQ_19211960_1day_FDCvar(1,:),ANA_CHO_MM_19612018_1day_FDCvar(2,:),[0 0.5 0],'Edgecolor','none','FaceAlpha',.1)
% Mean FDCs
...
disp('Exporting the Figure to pdf')
% Set the paper size [width height]
set(fig_S5,'PaperSize',[30 30]);
% set(fig_S5,'PaperOrientation','landscape')
print(fig_S5,'FigS5_export','-dpdf','-fillpage')

% Clear auxiliar variables
clear fig_S5 offset_loc refhyv refhydt refmon_hy refyrs lim1 lim2

disp('Done')
disp(' ')
%% 17. Figure: Regional effects
disp('17. Creating Figure of regional effects')
...
disp(' ')
%% 18. Save results
disp('18. Saving results from the analysis')
